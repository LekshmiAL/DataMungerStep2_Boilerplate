...
public class DataMunger {
	...
	public List<String> getLogicalOperators(String queryString) {
		...
		if(conditionsClause!=null && !conditionsClause.equals("")) {
			...
			Matcher matcher = Pattern.compile(" and ").matcher(conditionsClause);
			int andCount = 0;
			while (matcher.find()){
				conditionalOperator.add("and");
			}
			matcher = Pattern.compile(" or ").matcher(conditionsClause);
			int orCount = 0;
			while (matcher.find()){
				conditionalOperator.add("or");
			}
		}
